...
htmlcov/index.html: README.rst src/collective.sitemap/*.py bin/coverage
	@bin/coverage run --source=./src/collective.sitemap/ --branch bin/test
	@bin/coverage html -i
	@touch $@
...
docs/html/index.html: docs/*.rst src/collective.sitemap/*.py src/collective.sitemap/browser/*.py src/collective.sitemap/tests/*.py bin/sphinx-build
...
clean:
	@rm -rf .coverage .installed.cfg .mr.developer.cfg bin docs/html htmlcov parts develop-eggs \
		src/collective.sitemap.egg-info lib include .Python
